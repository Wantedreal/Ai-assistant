...
## Gantt Chart (PFE Report)

Two output files generated by `generate_gantt.py` at the project root:
- `GANTT_PFE.xlsx` — Excel with coloured phase/task bars (openpyxl)
- `GANTT_PFE.pdf` — A4 landscape PDF exported via Excel COM (single image ready to screenshot)

**To regenerate after any change:**
```powershell
.\backend\venv\Scripts\python.exe generate_gantt.py
```

**What to edit in `generate_gantt.py`:**

| What you want to change | Where |
|-------------------------|-------|
| Timeline start/end | `PFE_START` / `PFE_END` constants (top of file) |
| Add/remove a task | `PHASES` list — each task is `("Label", week_start, week_end, "status")` |
| Change task status | 4th element of the tuple: `"done"` / `"active"` / `"planned"` / `"milestone"` |
| Add a new phase | Add a new dict to `PHASES` with `"phase"`, `"color"`, `"light"`, `"tasks"` |
| Change colours | `"color"` (phase header hex) and `"light"` (task row background hex) per phase |
| Status colours/labels | `STATUS_META` dict near the top |
| Column widths | `ws.column_dimensions["A"].width` etc. or the `5.5` week column width |

**Week numbering:** weeks are 1-indexed from `PFE_START`. Week 1 = 23 Feb 2026, Week 26 = ~17 Aug 2026.

**Dependencies:** `openpyxl` (already in venv) + `pywin32` for PDF export (already installed). If PDF export fails, the xlsx is still saved.
...
- STEP export (`backend/app/step_export.py`): `POST /api/v1/export/step` — CadQuery parametric STEP of full cell array (cells, terminals, busbars, brackets/insulation cards/side plates). Performance: `Shape.moved()` + `Compound.makeCompound()` pattern — one OCC solid per unique shape, N cheap location-transforms (~60× faster than individual solids). Typical generation time: 1–7 s depending on cell count. Colors embedded as `COLOUR_RGB` in AP214; visible in SolidWorks/CATIA/Fusion 360/FreeCAD.
  - **Assembly tree:** top-level `battery_pack` → two child sub-assemblies: `couvercle` (6 named face plates: top/bottom/front/back/left/right, added only on ACCEPT verdict) + `module` (all cells, terminals, busbars, brackets, structural parts). Module never contains the couvercle.
  - **Couvercle geometry:** 6 separate box faces tiled with no volumetric overlap — top/bottom own the outer corners (`oL × wall × oW`), front/back fill inner height at ±Z (`oL × hH × wall`), left/right fill inner height and depth at ±X (`wall × hH × hW`). Inner cavity = configured housing dimensions exactly (hL × hH × hW). No Boolean solid → no z-fighting.
  - **Floor clearance:** cylindrical 3.5 mm (bracket protrudes 2.5 mm beyond terminal + 0.5 mm strip + 0.5 mm safety), prismatic/pouch 1.0 mm. Nickel strips placed flush against bracket outer face (no artificial gap).
  - **Dimension audit:** `_log_dim_check` logs a comparison table of every key STEP dimension vs engine/request values on every export — INFO for matches, WARNING for real mismatches.
...
- **STEP LRU cache** (`backend/app/step_export.py`): Module-level `_CACHE: OrderedDict[str, bytes]` with `_CACHE_MAX = 4`. Key computed via `hashlib` hash of all geometry parameters including `result.verdict.value` (ACCEPT and REJECT cached separately — couvercle present only on ACCEPT). Cache hit returns `BytesIO(cached)` immediately without re-running CadQuery. Reduces repeat generation from 1–7 s to instant for identical configurations.
...
- **ComparePanel benchmarking color logic** (`frontend/src/components/ComparePanel.jsx`): Spec rows are now colored by value agreement across all compared cells — **green** (`#4ade80`) when all selected cells share the same value for a row, **red** (`#f87171`) when values differ. Winner dot (amber ●) shown only on diff rows (not on same-value rows where there is no meaningful "best"). `rowStatuses` computed via `useMemo([selectedCells, filledCells.length])` iterating `SPEC_DEFS` — `'same'` | `'diff'` | `null` (single cell). Styles: `sameValue` and `diffValue` keys in `panelStyles` with subtle background tint.
